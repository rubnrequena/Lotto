package models
{
  ...
  public class VentasModel extends Model
  {
    ...
    public function VentasModel(owner:ModelHUB)
    {
      ...
      tmr = new Timer(5000);
      tmr.addEventListener(TimerEvent.TIMER,timer_commitHandler);
      tmr.start();
      ...
      Loteria.console.addEventListener(Console.PERMITIR_VENTAS,function permitirVentas_handler(permiso:Boolean):void {
        lockVentas = permiso
      })
      Loteria.console.addEventListener(Console.SINCRONIZAR,function sincronizar_handler(permiso:Boolean):void {
        if (permiso) tmr.start()
        else tmr.stop()
      })
      Loteria.console.addEventListener(Console.COMMIT,commitVentas)
    }
    
    private function commitVentas():void {
    ...
    private function timer_commitHandler (event:TimerEvent):void {
